...
int main()
{
    ...
    float spike8size = 40;
    // for spikes rectangle
    Vector2 spike8position = {6.7 * blocksize, 15.6 * blocksize};
    ...
    float spike9size = 40;
    // for spikes rectangle
    Vector2 spike9position = {7.17 * blocksize, 15.6 * blocksize};
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsKeyDown(KEY_RIGHT))
            ...
            speed.x = 0;
        if (IsKeyPressed(KEY_UP) && (onplatform || onring1 || onring2 || onring3))
        {
            speed.y = -jumpspeed;
            PlaySound(bounce);
        }
            ...
            position.x - currentradius < ring2right.x + ring2right.width)
        {
            ...
            checkpoint1 = true;
            checkpointposition = (Vector2){21.63 * blocksize, 6.7 * blocksize};
        ...
        /* POWER UP COLLISION */
        if (!poweruppass &&
            CheckCollisionCircles(position, currentradius, (Vector2){powerupposition.x + powerupsize / 2, powerupposition.y + powerupsize / 2}, powerupsize / 2))
        {
            poweruppass = true;
